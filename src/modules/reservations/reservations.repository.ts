...
export async function reserveBikeTx(stationId: number, userId: string) {
...
export async function returnBikeTx(stationId: number, userId: string, reservationId: string) {
  const client = await pool.connect();
  try {
    await client.query("BEGIN");
